...
class Lights(object):
    ...
    def __init__(self, green_pin, yellow_pin, red_pin):
        ...
        GPIO.setup(self.yellow_pin, GPIO.OUT) # Sagt dem GPIO Board, dass self.green_pin ein output Pin ist. Er also genutzt wird, um Spannungen zu erzeugen, nicht auszulesen.
        GPIO.setup(self.red_pin, GPIO.OUT) # Sagt dem GPIO Board, dass self.green_pin ein output Pin ist. Er also genutzt wird, um Spannungen zu erzeugen, nicht auszulesen.
# green
...
# yellow
    def turn_yellow_light_on(self):
        GPIO.output(self.yellow_pin, GPIO.HIGH) # Sagt dem GPIO Board, dass der Pin self.green_pin auf High (3,3V) zu setzen ist
    def turn_yellow_light_off(self):
        GPIO.output(self.yellow_pin, GPIO.LOW) # Sagt dem GPIO Board, dass der Pin self.green_pin auf Low (0V) zu setzen ist
#red
    def turn_red_light_on(self):
        GPIO.output(self.red_pin, GPIO.HIGH) # Sagt dem GPIO Board, dass der Pin self.green_pin auf High (3,3V) zu setzen ist
    def turn_red_light_off(self):
        GPIO.output(self.red_pin, GPIO.LOW) # Sagt dem GPIO Board, dass der Pin self.green_pin auf Low (0V) zu setzen ist
# off 
